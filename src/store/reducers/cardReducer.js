...
export const delete_card_product = createAsyncThunk(
    ...
    async (card_id, { rejectWithValue, fulfillWithValue}) => {
        try {
            const { data } = await api.get(`/home/product/delete-card-product/${card_id}`)
            return fulfillWithValue(data)
        } catch (error) {
            return rejectWithValue(error.response.data)
        }
...
export const quantity_inc = createAsyncThunk(
    ...
    async (card_id, { rejectWithValue, fulfillWithValue}) => {
        try {
            const { data } = await api.get(`/home/product/quantity-inc/${card_id}`)
            return fulfillWithValue(data)
        } catch (error) {
            return rejectWithValue(error.response.data)
        }
...
export const quantity_dec = createAsyncThunk(
    ...
    async (card_id, { rejectWithValue, fulfillWithValue}) => {
        try {
            const { data } = await api.get(`/home/product/quantity-dec/${card_id}`)
            return fulfillWithValue(data)
        } catch (error) {
            return rejectWithValue(error.response.data)
        }
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(add_to_card.fulfilled, (state, { payload }) => {
                ...
                state.card_products_count = payload.card_products_count + 1
            })
            ...
            .addCase(add_to_wishlist.fulfilled, (state, { payload }) => {
                state.successMessage = payload.message
                state.wishlist_count = payload.wishlist_count > 0 ? payload.wishlist_count + 1 : 1
            })
            ...
            .addCase(remove_wishlist.fulfilled, (state, { payload }) => {
                state.successMessage = payload.message
                state.wishlist = payload.wishlist.filter(p => p._id !== payload.wishlistId)
